...
**We began our modeling work here with a set seed to ensure we continue to get the same split no matter how much we run this chunk.  We split our data into training and testing to ensure we have extra data to test our model on, to prevent over fitting**
...
**To prep for our recipe and modeling, we wanted to peak at the distribution of our outcome variable, score, to see if we needed to up sample if it was lopsided (which is is).**
...
**Using our Lasso tune, we are able to collect and plot our accuracy values associated with our different penalty parameters to help visualize the smallest penalty we can use all while maximizing accuracy**
...
**We specifically hone in on our sole best model based on highest accuracy, and can pull its associated penalty parameter to use as our set penalty in our finalized model to come.**
...
**From this final model, we can also plot a variable importance plot and see which variables contributed the most and were the most influential when trying to predict score, with this matching the patterns we saw in the coefficients above.**
...
**Here we create our new observation representing the exact drive and scenario listed above and pipe in to display it.**
...
A major problem we had to deal with as it relates to modeling was form of the data.  We needed it to be an exact form with no NAs.  I was not sure how to uniformly replace NAs, doing so differently for different variables.  We had to read a lotttttt of documentation to figure it out but we got there eventually.  Additionally, when initially including passer (QB) in our dataset, we had a big problem when trying to model because the QB names changed yearly and were not the same throughout.  Each fold had concerns as there were new levels in nearly every step.  We had to learn about some new step functions within the lasso recipe that took care of this.  While we ended up scrapping this variable in our model, we also had to read a lot of documentation and talk with Professor Lendway in depth to uncover the error and address it.  Lastly, we ran into a time constraint with our random forest and boosted trees models.  We ended up showing overall metrics for these models and not going too much in to detail on purpose.  Besides the important fact that they weren't in the form we wanted, we also faced a lot of issues with the way our data was presented.  After trying for days, we were unable to finalize models for the rf and boosted models, after tweaking the data so many different ways and trying so many different types of steps and up and down sampling in our recipe.

If we had a couple more weeks, we also would have provided an in depth analysis, with tree visualizations, for our random forest and boosted models as we did for our lasso log reg.  


# Most Proud Part

Ben: The plot showing each quarterbacks number of successful drives compared to total 2-minute drill drives is my most proud point of the project. Using each players headshot, I plotted the number of successful 2-minute drill drives each QB in the league has completed compared to how many drives they have attempted. I have never used an image to plot the data and it took a lot of cleaning and fixing before each headshot matched up with the right QB. Some QBs even showed up multiple times. However, after fixing the data in the cleaning section, I was able to get it to look really nice, and I think it is one of my most professional plots of my time here at Mac. One thing I could have changed is the y-axis. I think I could have made the plot a little bit more informative if I put another more interesting variable on the y-axis. Nonetheless, I think it still turned out great.  

Anthony: My most proud part of the code I focused on was the lasso log reg.  We had errors in so many different places so many different times.  It took days to sort out what was going on and was quite frustrating especially considering the basic nature of the modeling we were attempting.  After finding some crafty ways to manipulate within the recipe, removing NAs, and even removing whole variables that were tipping the model off on the outcome making it wayyyyy to accurate (like fg and td as a proxy for score), we were finally able to finalize a workflow, model, and use the finalized forms to see some really cool and in depth details about our desired model.  