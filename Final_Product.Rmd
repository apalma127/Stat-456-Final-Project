...
```{r}
new_qb <- tribble(~qbr_raw, ~qbr_total, ~pass_tot_yds, ~tot_yds, ~ydsnet, ~rush_yds_tot, ~completion_perc, ~run_plays, ~pass_plays, ~drive_yards_penalized, ~drive_game_clock_start, ~yards_to_go_start, ~score,
                     70, 75, 50, 70, 80, 20, 0.6, 4, 7, 20, 115, 90, 1)
new_qb %>%
  kbl() %>% 
  kable_paper("striped") %>% 
