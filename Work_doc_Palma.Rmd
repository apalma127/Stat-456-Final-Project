  ...
  two_min_new  %>%
    ...
    mutate(run_plays = sum(rush_attempt, na.rm = TRUE), 
     ...
     mutate(td = ifelse(fixed_drive_result=='Touchdown', 1, 0),
            fg = ifelse(fixed_drive_result=='Field goal', 1, 0), 
            score = ifelse(td+fg==1, 1,0)) %>%
    select(passer, pass_tot_yds, tot_yds, rush_yds_tot, rusher, completion_perc, run_plays, pass_plays, penalties, drive_yards_penalized, tot_yds, drive_game_clock_start, td, fg, score)
```
...
```
```{r}
drive_summary_data$rusher[is.na(drive_summary_data$rusher)] <- 0
...
```{r}
drive_summary_data %>%
  select(game_id, passer, rusher, completion_perc)
```
