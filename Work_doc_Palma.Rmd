...
```{r}
two_min_new %>%
  select(passer, game_id, rusher, ydsnet)
```
  ...
  two_min_new  %>%
    ...
    mutate(run_plays = sum(rush_attempt, na.rm = TRUE), 
     ...
     mutate(td = ifelse(fixed_drive_result=='Touchdown', 1, 0),
            fg = ifelse(fixed_drive_result=='Field goal', 1, 0), 
            score = ifelse(td+fg==1, 1,0)) %>%
    select(passer, pass_tot_yds, tot_yds, ydsnet, rush_yds_tot, rusher, completion_perc, run_plays, pass_plays, penalties, drive_yards_penalized, tot_yds, drive_game_clock_start, td, fg, score)
```
...
```{r}
drive_summary_data %>%
  select(game_id, passer, rusher, completion_perc, game_week, week_num)
```

* just QBR and QB, no rusher
...
```{r}
drive_summary_data %>%  
  ggplot(aes(y = completion_perc, x = pass_tot_yds)) +
  geom_point()
```
